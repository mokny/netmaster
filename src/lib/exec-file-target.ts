...
export async function resolveDockerFileBackend(
  ...
  if (!/^[a-zA-Z0-9]+$/.test(containerId)) throw new ExecTargetError(400, "Ungültige Container-ID");
...
export async function resolveProxmoxFileBackend(
  ...
  if (!Number.isInteger(vmid) || vmid <= 0) throw new ExecTargetError(400, "Ungültige VM-ID");
  const server = await prisma.server.findUnique({ where: { id: serverId } });
  if (!server) throw new ExecTargetError(404, "Server nicht gefunden");
